...
\usepackage{subcaption} % Um mehrere Bilder in eine figure einzufügen - subfigure ist outdated
...
\usepackage{adjustbox} % ähnlich wie ResizeBox (plus centering)
\usepackage{multirow} %Tabellenfelder über 2 Zeilen
\usepackage{appendix}
...
\newcommand{\plm}{$\pm$} % Für automatisch erzeugten LaTeX Code (Plus-Minus Zeichen als Abkürzung)
...
\todo{Github TF-Issue Datum in Quellen anpassen}
...
\appendix
\addappheadtotoc
\let\cleardoublepage\clearpage % Ansonsten werden zwischen Anhangkapiteln **2** aufeinanderfolgende Leerseiten, also ein beidseitig leeres Blatt, eingefügt.
\include{chapter/Anhang}

% Eidesstattliche Erklärung
\todo{Kapitelstart und Eidesstattliche Erklärung auf gerade / ungerade Seitenzahl prüfen}
